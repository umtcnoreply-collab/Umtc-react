    ...
    return (
      <tr className="border-b border-[#8c707133] hover:bg-[#e5e2dd]/30 transition-colors">
        ...
        <td className="p-2 w-[14%]" style={{minWidth: '110px'}}>
          {data.file ? (
            <div className="flex items-center justify-between bg-[#570013]/10 border border-[#570013] p-2 rounded gap-2">
              <span className="text-xs font-semibold text-[#570013] truncate flex items-center gap-1" title={typeof data.file === 'object' && 'name' in data.file ? (data.file as any).name : (data.file as File).name || 'uploaded'}>
              ...
              <button
                ...
                className="p-1 hover:bg-[#ba1a1a] hover:text-white rounded transition-colors flex-shrink-0"
              >
    ...
    return (
      <div className="bg-[#f0ede8] p-5 rounded-xl border border-white/50 space-y-4 shadow-sm">
        ...
        <div className="space-y-4">
          ...
          <div className="pt-2">
            {data.file ? (
              <div className="flex items-center justify-between bg-[#570013]/10 border border-[#570013] p-3 rounded">
                <span className="text-sm font-semibold text-[#570013] truncate flex items-center gap-2" title={typeof data.file === 'object' && 'name' in data.file ? (data.file as any).name : (data.file as File).name || 'uploaded'}>
                ...
                <button
                  type="button"
                  onClick={() => handleQualChange(level, 'file', null)}
                  className="p-1.5 hover:bg-[#ba1a1a] hover:text-white rounded transition-colors flex-shrink-0"
                >
      ...
      <div className="md:hidden flex-grow pt-20 px-4 pb-32 max-w-md mx-auto w-full">
        ...
        <form className="space-y-10" onSubmit={handleSubmit}>
          ...
          <section className="space-y-6">
            ...
            <div className="bg-[#f0ede8] p-6 rounded-2xl shadow-sm border border-white/50 space-y-6">
              <div className="space-y-3">
                ...
                {photoPreview ? (
                  <div className="w-full relative">
                    <img src={photoPreview} alt="Photo Preview" className="w-full h-48 object-cover rounded-lg border-2 border-[#570013]" />
                    <button
                      type="button"
                      onClick={clearPhoto}
                      className="absolute top-2 right-2 bg-[#ba1a1a] text-white p-2 rounded-full hover:bg-[#d32f2f] transition-colors"
                    >
                      <span className="material-symbols-outlined text-sm">close</span>
                    </button>
                  </div>
                ) : (
                  <label className="w-full h-48 bg-[#ebe8e3] rounded-lg flex flex-col items-center justify-center border-2 border-dashed border-[#8c7071]/50 cursor-pointer hover:border-[#570013] transition-colors">
              ...
              <div className="space-y-3">
                ...
                {signaturePreview ? (
                  <div className="w-full relative">
                    <img src={signaturePreview} alt="Signature Preview" className="w-full h-32 object-cover rounded-lg border-2 border-[#570013]" />
                    <button
                      type="button"
                  ...
                  <label className="w-full h-32 bg-[#ebe8e3] rounded-lg flex flex-col items-center justify-center border-2 border-dashed border-[#8c7071]/50 cursor-pointer hover:border-[#570013] transition-colors">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-[#570013] text-white py-5 rounded-xl font-bold tracking-widest shadow-xl flex items-center justify-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {loading ? 'UPLOADING...' : 'SAVE & PREVIEW'}
            {!loading && <span className="material-symbols-outlined">arrow_forward</span>}
          </button>
          {error && (
            <div className="w-full bg-[#ffebee] border border-[#ba1a1a] text-[#ba1a1a] p-4 rounded-lg text-sm font-semibold">
              {error}
            </div>
          )}
        </form>
          ...
          <div className="flex-1 overflow-x-hidden">
            <div className="mb-16">
              <div className="flex items-center justify-between relative">
                ...
                <div className="flex flex-col items-center gap-2">
                  <div className="w-10 h-10 rounded-full bg-[#570013] text-white flex items-center justify-center font-bold shadow-md">3</div>
                  <span className="text-sm font-['Inter'] text-[#570013] font-semibold">Documents</span>
                </div>
            ...
            <form className="space-y-16" onSubmit={handleSubmit}>
              ...
              <section className="grid grid-cols-[1fr_2.5fr] gap-12">
                ...
                <div className="bg-[#f0ede8] p-10 rounded-2xl shadow-sm border border-white/50 flex gap-12">
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    {photoPreview ? (
                      <div className="h-64 bg-white rounded-xl border-2 border-[#570013] overflow-hidden relative group">
                        <img src={photoPreview} alt="Photo Preview" className="w-full h-full object-cover" />
                        <button
                          type="button"
                          onClick={clearPhoto}
                          className="absolute top-3 right-3 bg-[#ba1a1a] text-white p-2 rounded-full hover:bg-[#d32f2f] transition-colors shadow-lg"
                        >
                          <span className="material-symbols-outlined text-base">close</span>
                        </button>
                      </div>
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    {signaturePreview ? (
                      <div className="h-40 bg-white rounded-xl border-2 border-[#570013] overflow-hidden relative group">
                  ...
                  <table className="w-full text-left border-collapse min-w-[1000px]">
                    <thead>
                      <tr className="bg-[#e5e2dd] border-b-2 border-[#8c7071]">
                        <th className="p-3 font-bold text-xs uppercase tracking-wider text-[#570013] w-[17%]">Qualification</th>
                        <th className="p-3 font-bold text-xs uppercase tracking-wider text-[#570013] w-[20%]">Board/University</th>
                        <th className="p-3 font-bold text-xs uppercase tracking-wider text-[#570013] w-[17%]">Passing Yr</th>
                        <th className="p-3 font-bold text-xs uppercase tracking-wider text-[#570013] w-[15%]">Roll No.</th>
                        <th className="p-3 font-bold text-xs uppercase tracking-wider text-[#570013] w-[11%]">Obtained</th>
                        <th className="p-3 font-bold text-xs uppercase tracking-wider text-[#570013] w-[11%]">Total</th>
                        <th className="p-3 font-bold text-xs uppercase tracking-wider text-[#570013] w-[11%]">Percentage</th>
              ...
              {/* SUBMIT */}
              <div className="pt-6 border-t border-[#8c707133] flex justify-end gap-4 flex-col">
                {error && (
                  <div className="w-full bg-[#ffebee] border border-[#ba1a1a] text-[#ba1a1a] p-4 rounded-lg text-sm font-semibold">
                    {error}
                  </div>
                ...
                <button 
                  className="bg-[#570013] text-white py-5 px-10 rounded-lg font-['Public_Sans'] font-bold text-lg uppercase tracking-widest shadow-xl hover:bg-[#800020] transition-all flex items-center justify-center gap-3 group disabled:opacity-60 disabled:cursor-not-allowed" 
                  type="submit"
                  disabled={loading}
                >
                  {loading ? 'UPLOADING...' : 'Save & Preview'}
